...
import { Search, Plus, Download, MoreHorizontal, Edit, Trash2, Filter, Upload } from "lucide-react";
  ...
  const handleImportMembers = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of dataRows) {
        try {
          const [fullName, email, phone, membershipType, dateOfBirth, gender, address, city, state, postalCode, country] = row;

          const memberId = await generateMemberId(membershipType?.trim() || "Extra");
          ...
          const { error } = await supabase
            ...
            .insert([{
              ...
              email: email?.trim(),
              phone: phone?.trim(),
              membership_type: membershipType?.trim() || "Extra",
              status: "active",
              date_of_birth: dateOfBirth?.trim() || null,
              gender: gender?.trim()?.toLowerCase() || "male",
      ...
      toast({
        ...
        description: `Successfully imported ${successCount} members. ${errorCount} errors.`,
      });
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="hidden"
            />
            <Button variant="outline" onClick={() => document.getElementById('import-members')?.click()}>
